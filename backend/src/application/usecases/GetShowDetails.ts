 ...
 * Combines data from external API with local state (watched) from database.
 */
export class GetShowDetails {
  ...
  async execute(showId: number): Promise<ShowDetails> {
    // Fetch show data and episodes in parallel for better performance
    ...
    // Fetch the "watched" state saved locally in the database
    const savedEpisodes = await this.episodeRepository.findByShowId(showId);

    // Create a map for fast lookup: episodeId -> watched
